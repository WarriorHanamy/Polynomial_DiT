#!/usr/bin/env python3
"""
Polynomial trajectory generation demo.
Visualizes multiple samples comparing predicted polynomial trajectories with reference points (3D + XY projection).
"""
...
def create_simplified_demo(model_path="checkpoints/Polynomial-DiT-local/grasp_checkpoint_latest.pth", 
                          processed_data_file="datasets/grasp_processed.txt",
                          save_dir="logs",
                          fig_width=15,
                          fig_height=5,
                          left_margin=0.06,
                          ...
                          legend_y=1.0):
    """
    Create a simplified demo figure with 3 2D plots.
    Each subplot shows the predicted polynomial trajectory and the input point,
    with an inclined plane box and slope angle annotation.

    Args:
        model_path: Path to the trained model checkpoint.
        processed_data_file: Path to the preprocessed dataset file.
        num_figures: Number of samples to visualize (fixed to 3).
        save_dir: Directory to save the generated figures.
        Other args: Layout parameters for figure appearance.
    """
    print("=== Creating simplified demo (predicted polynomials + input point + slope angle) ===")
    ...
    evaluator = PolynomialEvaluator(model_path)
    
    dataset = TrajectoryDataset(processed_data_file, normalize=True, target_type="polynomial")
    ...
    for fig_idx, data_idx in enumerate(selected_indices):
        condition, true_coeffs = dataset[data_idx]
        ...
        pred_coeffs = evaluator.predict_polynomial_coeffs(condition_batch)
        pred_coeffs_denorm = coeff_scaler.inverse_transform(pred_coeffs.cpu().numpy())[0]
        
        pred_coeffs_reshaped = pred_coeffs_denorm.reshape(3, 5)
        
        t_dense = np.linspace(0, 1, 100)
        pred_trajectory = dataset.polynomial_to_trajectory(pred_coeffs_reshaped, t_dense)
        
        condition_denorm = condition_scaler.inverse_transform(condition_np.reshape(1, -1))[0]
        
        all_x_coords.extend(pred_trajectory[:, 0])
        all_x_coords.append(condition_denorm[0])
        
        all_y_coords.extend(pred_trajectory[:, 1])
    ...
    for fig_idx, data in enumerate(all_trajectory_data):
        ...
        ax.plot(data['pred_trajectory'][:, 0], data['pred_trajectory'][:, 1], 
                color=morandi_colors['pred_poly'], linewidth=4, alpha=0.9,
                label='Predicted Polynomial' if fig_idx == 0 else '')
    ...
    legend_elements = [
        plt.Line2D([0], [0], color=morandi_colors['pred_poly'], linewidth=4, label='Predicted Polynomial'),
    ...
    save_path = os.path.join(save_dir, 'simplified_demo.png')
    fig.savefig(save_path, dpi=dpi, bbox_inches='tight', 
                facecolor='white', edgecolor='none', pad_inches=0.1)
    
    print("Simplified demo figure saved to:")
...
if __name__ == "__main__":
    print("=== Creating simplified demo figure ===")
